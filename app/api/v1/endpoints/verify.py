...
async def verify_growth(
    ...
    if ai_result["status"] == VerificationStatus.APPROVED:
        ...
        base_points = settings.points_per_verification
        multiplier = 1
        ward_id = spot.ward_name.lower().replace(" ", "_")
        try:
            ...
            log.warning("verify.points_multiplier_failed", error=str(exc))
    ...
    return VerificationResult(
        ...
        height_cm=ai_result.get("height_cm"),
        health_score=ai_result.get("health_score"),
        care_tip=ai_result.get("care_tip"),
