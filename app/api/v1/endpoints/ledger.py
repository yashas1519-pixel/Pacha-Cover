...
async def adopt_spot(
    ...
    # ── AI Dynamic Points Multiplier ──────────────────────────────────────────
    # Ward with CRITICAL heat risk gets 3x points. HIGH gets 2x.
    base_points = settings.points_per_adoption
    multiplier = 1
    ward_id = payload.ward_name.lower().replace(" ", "_")
    
    try:
        from app.models.firestore_collections import Collections, FirestoreFields
        ward_ref = db.collection(Collections.WARD_HEAT_DATA).document(ward_id)
        ward_doc = await ward_ref.get()
        if ward_doc.exists:
            ward_data = ward_doc.to_dict() or {}
            heat_score = ward_data.get(FirestoreFields.HEAT_RISK_SCORE, 0)
            if heat_score >= 75:  # CRITICAL
                multiplier = 3
            elif heat_score >= 50:  # HIGH
                multiplier = 2
    except Exception as exc:
        log.warning("ledger.adopt_points_multiplier_failed", error=str(exc))
        
    points_awarded = base_points * multiplier

    try:
        spot = await ledger.adopt_spot(
            user_id=uid, payload=payload, points_awarded=points_awarded
        )
    except Exception as exc:
