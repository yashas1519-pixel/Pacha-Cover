...
Return a JSON object with this exact structure:
{
    ...
    "height_cm": 45,
    "health_score": 85,
    "care_tip": "Add mulch to retain moisture in this heat.",
    "reasoning": "Brief explanation of why it was approved or rejected"
}
Note: 'status' MUST be exactly either "approved" or "rejected". If rejected, 'species', 'height_cm', 'health_score', and 'care_tip' can be null.
"""
...
class GeminiService(BaseAiService):
    ...
    # ── Conversational Spot Assistant ──────────────────────────────────────────

    async def chat_prescribe(self, request: ChatPrescriptionRequest, site_health: dict | None = None) -> ChatPrescriptionResponse:
        """
        Conversational assistant to ask clarifying questions before prescribing.
        """
        chat_instruction = (
            "You are a helpful, conversational urban forestry assistant in Bengaluru. "
            "Your goal is to recommend a tree species for a user's location.\n"
            "Ask clarifying questions one by one (e.g. sun exposure, space, water) if needed.\n"
            "If you have enough information to make a recommendation, return a structured prescription instead.\n\n"
            "Return JSON matching this schema:\n"
            "{\n"
            '  "is_complete": false,\n'
            '  "reply": "Your next question or response here",\n'
            '  "prescription": null\n'
            "}\n"
            "If complete:\n"
            "{\n"
            '  "is_complete": true,\n'
            '  "reply": "Here is your recommendation!",\n'
            '  "prescription": {\n'
            '    "primary_recommendation": { "common_name": "...", "scientific_name": "...", "why_recommended": "..." },\n'
            '    "alternative_recommendations": [ { "common_name": "...", "scientific_name": "...", "why_recommended": "..." } ]\n'
            '  }\n'
            "}"
        )
        ...
        for msg in request.messages:
            history.append({"role": "model" if msg.role == "model" else "user", "parts": [msg.content]})

        context = f"Context: Lat {request.coordinates.latitude}, Lng {request.coordinates.longitude}."
        if site_health:
             context += f" Site NDVI: {site_health.get('ndvi')}. LST: {site_health.get('lst_celsius')} C."

        if history:
             history[0]["parts"][0] = context + "\n" + history[0]["parts"][0]
        else:
             history.append({"role": "user", "parts": [context + "\nHi, I want to plant a tree."]})
        ...
        presc_resp = None
        if result.get("is_complete") and result.get("prescription"):
            p_data = result["prescription"]
            
            primary_data = p_data.get("primary_recommendation", {})
            primary = TreeSpecies(
                common_name=primary_data.get("common_name", "Unknown"),
                scientific_name=primary_data.get("scientific_name", "Unknown"),
                why_recommended=primary_data.get("why_recommended", "")
            )
            
            alts = []
            for alt in p_data.get("alternative_recommendations", []):
                 alts.append(TreeSpecies(
                     common_name=alt.get("common_name", "Unknown"),
                     scientific_name=alt.get("scientific_name", "Unknown"),
                     why_recommended=alt.get("why_recommended", "")
                 ))
                 
            presc_resp = PrescriptionResponse(
        ...
        return ChatPrescriptionResponse(
            is_complete=result.get("is_complete", False),
            reply=result.get("reply", "I'm having trouble understanding."),
            prescription=presc_resp
        )
    ...
    async def verify_sapling_image(
        ...
        return {
            ...
            "species": result.get("species") if 'result' in locals() else None,
            "height_cm": result.get("height_cm") if 'result' in locals() else None,
            "health_score": result.get("health_score") if 'result' in locals() else None,
            "care_tip": result.get("care_tip") if 'result' in locals() else None,
            "gcs_uri": None,
            "message": message,
        }
