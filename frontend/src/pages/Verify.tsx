    ...
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      ...
      {result && (
        <div className="glass-panel" style={{ border: `1px solid ${approved ? 'var(--accent-green)' : '#ef4444'}`, animation: 'fadeIn 0.5s ease-out' }}>
          ...
          {approved && result.species && (
            <div style={{ padding: '1.25rem', background: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid var(--glass-border)', marginBottom: '1.5rem', textAlign: 'left' }}>
              <h4 style={{ margin: '0 0 1rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-primary)', fontFamily: 'Outfit, sans-serif' }}>
                <Leaf size={18} color="var(--accent-green)" /> AI Arborist Report
              </h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Confirmed Species</div>
                  <div style={{ fontSize: '1.05rem', fontWeight: 600 }}>{result.species}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Estimated Height</div>
                  <div style={{ fontSize: '1.05rem', fontWeight: 600 }}>{result.height_cm ? `${result.height_cm} cm` : 'N/A'}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Health Score</div>
                  <div style={{ fontSize: '1.05rem', fontWeight: 600, color: result.health_score > 80 ? 'var(--accent-green)' : result.health_score > 50 ? '#f59e0b' : '#ef4444' }}>
                    {result.health_score ? `${result.health_score}/100` : 'N/A'}
                  </div>
                </div>
              </div>
              {result.care_tip && (
                <div style={{ padding: '0.75rem', background: 'rgba(34,197,94,0.05)', borderLeft: '3px solid var(--accent-green)', borderRadius: '0 8px 8px 0', fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                  <strong style={{ color: 'var(--text-primary)' }}>Care Tip:</strong> {result.care_tip}
                </div>
              )}
            </div>
          )}
          ...
          {approved && (
            <div style={{ padding: '1.5rem', background: 'linear-gradient(135deg, rgba(34,197,94,0.12), rgba(16,185,129,0.06))', borderRadius: '16px', textAlign: 'center', border: '1px solid rgba(34,197,94,0.2)' }}>
              ...
              <div style={{ fontSize: '3rem', fontWeight: 800, lineHeight: 1.1, fontFamily: 'Outfit, sans-serif', background: 'linear-gradient(135deg, #22c55e, #84cc16)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                +{result.green_points_awarded}
              </div>
              {result.green_points_awarded > 50 && (
                <div style={{ display: 'inline-block', padding: '0.25rem 0.75rem', background: '#f59e0b', color: '#000', fontWeight: 700, fontSize: '0.75rem', borderRadius: '999px', marginTop: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  🔥 Multiplier Active
                </div>
              )}
              <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '0.5rem' }}>Added to your Green Ledger</div>
            </div>
          )}
